...
function evg_marketplace_tab() {
    ...
    // ---------------------------------------------------------
    // Handle Form Submissions (Create / Edit Listing)
    // ---------------------------------------------------------
    if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['evg_save_listing_nonce'] ) ) {
        if ( wp_verify_nonce( $_POST['evg_save_listing_nonce'], 'evg_save_listing' ) ) {
            ...
            $grading_company  = sanitize_text_field( $_POST['grading_company'] );
            $slab_information = sanitize_text_field( $_POST['slab_information'] );
            $assigned_grade   = ! empty( $_POST['assigned_grade'] ) ? intval( $_POST['assigned_grade'] ) : null;
            ...
            $formats = array( '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%f', '%d', '%s', '%s' );
            ...
            if ( $listing_id > 0 ) {
                ...
                echo '<div class="notice notice-success is-dismissible" style="background:#141416; border-left:4px solid #d4af37; color:#fff; padding:12px 16px; margin-bottom:20px; border-radius:6px;"><p style="margin:0; font-weight:600;">' . esc_html__( 'Marketplace listing updated successfully.', 'evg-platform' ) . '</p></div>';
            } else {
                $data['listed_date'] = current_time( 'mysql' );
                $formats[] = '%s';
                $wpdb->insert( $table_marketplace, $data, $formats );
                $new_id = $wpdb->insert_id;
                Elite_Vault_Grading_System::log_activity( "Created Marketplace Listing #{$new_id} ({$card_title})." );
                echo '<div class="notice notice-success is-dismissible" style="background:#141416; border-left:4px solid #d4af37; color:#fff; padding:12px 16px; margin-bottom:20px; border-radius:6px;"><p style="margin:0; font-weight:600;">' . esc_html__( 'Card published to public marketplace catalogue.', 'evg-platform' ) . '</p></div>';
            }
        }
    }

    // ---------------------------------------------------------
    // Handle Quick Status Toggles (Sold / Available)
    // ---------------------------------------------------------
    if ( isset( $_GET['toggle_status'] ) && isset( $_GET['listing_id'] ) && isset( $_GET['_wpnonce'] ) ) {
        $listing_id = intval( $_GET['listing_id'] );
        if ( wp_verify_nonce( $_GET['_wpnonce'], 'evg_toggle_status_' . $listing_id ) ) {
            ...
            echo '<div class="notice notice-success is-dismissible" style="background:#141416; border-left:4px solid #d4af37; color:#fff; padding:12px 16px; margin-bottom:20px; border-radius:6px;"><p style="margin:0; font-weight:600;">' . esc_html__( 'Listing status updated.', 'evg-platform' ) . '</p></div>';
        }
    }

    // ---------------------------------------------------------
    // Handle Deletion (Purge Listing)
    // ---------------------------------------------------------
    if ( isset( $_GET['delete_listing'] ) && isset( $_GET['_wpnonce'] ) ) {
        if ( wp_verify_nonce( $_GET['_wpnonce'], 'evg_delete_listing_' . intval( $_GET['delete_listing'] ) ) ) {
            $del_id = intval( $_GET['delete_listing'] );
            ...
            echo '<div class="notice notice-success is-dismissible" style="background:#141416; border-left:4px solid #ff453a; color:#fff; padding:12px 16px; margin-bottom:20px; border-radius:6px;"><p style="margin:0; font-weight:600;">' . esc_html__( 'Listing removed from marketplace.', 'evg-platform' ) . '</p></div>';
        }
    }
...
function evg_render_pro_marketplace_list( $table_marketplace ) {
    ...
    <div class="evg-mkt-header">
        <div>
            <h1><?php esc_html_e( 'Public Marketplace Slabs', 'evg-platform' ); ?></h1>
            <p><?php esc_html_e( 'Manage live certified inventory, price valuations, stock levels, and marketplace listings.', 'evg-platform' ); ?></p>
        </div>
        <div>
            <a href="<?php echo esc_url( admin_url( 'admin.php?page=evg_tab_marketplace&action=add' ) ); ?>" class="evg-btn-add">
                + <?php esc_html_e( 'Add Marketplace Listing', 'evg-platform' ); ?>
            </a>
        </div>
    </div>
        ...
        <table class="evg-mkt-table evg-datatable">
            <thead>
                <tr>
                    ...
                    <th><?php esc_html_e( 'Card Specification & Slab', 'evg-platform' ); ?></th>
                    <th><?php esc_html_e( 'Category', 'evg-platform' ); ?></th>
                    <th><?php esc_html_e( 'Stock', 'evg-platform' ); ?></th>
                    <th><?php esc_html_e( 'Valuation (£)', 'evg-platform' ); ?></th>
            ...
            <tbody>
                <?php if ( ! empty( $listings ) ) : ?>
                    <?php foreach ( $listings as $item ) : 
                        ...
                        $display_title = $item->card_title ? $item->card_title : 'Graded Pokémon Card';
                    ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                ...
                                <?php if ( $item->assigned_grade ) : ?>
                                    <span class="evg-grade-chip"><?php echo esc_html( $item->grading_company . ' ' . $item->assigned_grade ); ?></span>
                                <?php endif; ?>
                                <br>
                            ...
                            <td>
                                <strong style="color: <?php echo $item->stock_quantity > 0 ? '#34c759' : '#ff453a'; ?>;">
                                    <?php echo esc_html( $item->stock_quantity ); ?> Units
                                </strong>
                            </td>
                            <td>
                            ...
                            <td style="text-align: right;">
                                <?php if ( $item->status === 'Available' ) : ?>
                                    <a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=evg_tab_marketplace&toggle_status=Sold&listing_id=' . $item->id ), 'evg_toggle_status_' . $item->id ) ); ?>" class="evg-btn-quick-toggle" title="<?php esc_attr_e( 'Mark as Sold', 'evg-platform' ); ?>">
                                        <?php esc_html_e( 'Mark Sold', 'evg-platform' ); ?>
                                    </a>
                                <?php else : ?>
                                    <a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=evg_tab_marketplace&toggle_status=Available&listing_id=' . $item->id ), 'evg_toggle_status_' . $item->id ) ); ?>" class="evg-btn-quick-toggle" title="<?php esc_attr_e( 'Mark Available', 'evg-platform' ); ?>">
                                        <?php esc_html_e( 'Make Available', 'evg-platform' ); ?>
                                    </a>
                                <?php endif; ?>
                                ...
                                <a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=evg_tab_marketplace&delete_listing=' . $item->id ), 'evg_delete_listing_' . $item->id ) ); ?>" class="evg-btn-action-del" onclick="return confirm('<?php esc_attr_e('Permanently remove this card from the public marketplace?', 'evg-platform'); ?>');">
                                    ✕
                                </a>
                            </td>
                        </tr>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="7" style="text-align: center; color: var(--evg-text-muted); padding: 50px;">
                            <?php esc_html_e( 'No cards currently listed on the public marketplace.', 'evg-platform' ); ?>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
    ...
    <script>
        jQuery(document).ready(function($) {
            $('.evg-datatable').DataTable({
                "pageLength": 25,
                "order": [[ 4, "desc" ]],
                "language": { "search": "Search Listings:" }
            });
        });
    </script>
    <?php
    ...
    <div class="evg-mkt-header">
        <div>
            <a href="<?php echo esc_url( admin_url( 'admin.php?page=evg_tab_marketplace' ) ); ?>" style="color: #d4af37; text-decoration: none; font-size: 13px; font-weight: 700; margin-bottom: 8px; display: inline-block;">
                ← <?php esc_html_e( 'Back to Marketplace Ledger', 'evg-platform' ); ?>
            </a>
            <h1><?php echo $listing ? esc_html__( 'Modify Marketplace Listing', 'evg-platform' ) : esc_html__( 'Add New Marketplace Slab', 'evg-platform' ); ?></h1>
            <p><?php esc_html_e( 'Configure inventory details, grading company, slab info, stock, and price valuations.', 'evg-platform' ); ?></p>
        </div>
    </div>

    <div class="evg-form-container">
        <div class="evg-form-header">
            <h2><?php esc_html_e( 'Listing Parameters', 'evg-platform' ); ?></h2>
        </div>
        <div class="evg-form-body">
            <form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=evg_tab_marketplace' ) ); ?>">
                ...
                <div class="evg-input-group">
                    <label><?php esc_html_e( 'Import Graded Card from System (Optional)', 'evg-platform' ); ?></label>
                    <select id="evg_graded_card_picker" name="card_id" class="evg-control-field">
                        <option value=""><?php esc_html_e( '— Standalone Listing / Custom Inventory —', 'evg-platform' ); ?></option>
                ...
                <div class="evg-input-group">
                    <label><?php esc_html_e( 'Card Title / Name', 'evg-platform' ); ?></label>
                ...
                <div class="evg-input-grid-2">
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Set Name', 'evg-platform' ); ?></label>
                ...
                <div class="evg-input-grid-2">
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Card Language', 'evg-platform' ); ?></label>
                    ...
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Grading Company', 'evg-platform' ); ?></label>
                ...
                <div class="evg-input-grid-2">
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Assigned Grade (1-10 Whole Number)', 'evg-platform' ); ?></label>
                        <select id="mkt_assigned_grade" name="assigned_grade" class="evg-control-field">
                            <option value=""><?php esc_html_e( 'Ungraded / Raw', 'evg-platform' ); ?></option>
                            <?php for ( $g = 10; $g >= 1; $g-- ) : ?>
                                <option value="<?php echo esc_attr( $g ); ?>" <?php if ( $listing ) selected( $listing->assigned_grade, $g ); ?>>
                                    <?php echo esc_html( $g ); ?> - Grade <?php echo esc_html( $g ); ?>
                                </option>
                            <?php endfor; ?>
                        </select>
                    </div>
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Slab Information / Style', 'evg-platform' ); ?></label>
                        <input type="text" name="slab_information" class="evg-control-field" value="<?php echo $listing ? esc_attr( $listing->slab_information ) : 'Elite Vault Sealed Protective Slab'; ?>" placeholder="e.g. Standard Slab / Shield Premium Slab">
                    </div>
                </div>

                <div class="evg-input-group">
                    <label><?php esc_html_e( 'Card Display Image URL', 'evg-platform' ); ?></label>
                ...
                <div class="evg-input-grid-2">
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Retail Price (£)', 'evg-platform' ); ?></label>
                    ...
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Stock Quantity', 'evg-platform' ); ?></label>
                ...
                <div class="evg-input-grid-2">
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Catalogue Category', 'evg-platform' ); ?></label>
                    ...
                    <div class="evg-input-group">
                        <label><?php esc_html_e( 'Listing Status', 'evg-platform' ); ?></label>
                        <select name="status" class="evg-control-field" required>
                            <option value="Available" <?php if ( $listing ) selected( $listing->status, 'Available' ); ?>><?php esc_html_e( 'Available (Published)', 'evg-platform' ); ?></option>
                            <option value="Sold" <?php if ( $listing ) selected( $listing->status, 'Sold' ); ?>><?php esc_html_e( 'Sold (Out of Stock)', 'evg-platform' ); ?></option>
                            <option value="Hidden" <?php if ( $listing ) selected( $listing->status, 'Hidden' ); ?>><?php esc_html_e( 'Hidden (Internal Draft)', 'evg-platform' ); ?></option>
                        </select>
                    </div>
                </div>

                <button type="submit" class="evg-btn-publish">
                    <?php echo $listing ? esc_html__( 'Update Listing Parameters', 'evg-platform' ) : esc_html__( 'Publish to Public Marketplace', 'evg-platform' ); ?>
                </button>
            </form>
        </div>
    </div>
